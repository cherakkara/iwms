...
public abstract class AbstractSessionOutputBuffer implements BufferInfo, SessionOutputBuffer {
    ...
    protected void init(OutputStream outputStream, int i, HttpParams httpParams) {
        ...
        this.ascii = this.charset.equals(Consts.ASCII);
        this.encoder = null;
        this.minChunkLimit = httpParams.getIntParameter(CoreConnectionPNames.MIN_CHUNK_LIMIT, AccessibilityNodeInfoCompat.ACTION_PREVIOUS_AT_MOVEMENT_GRANULARITY);
        this.metrics = createTransportMetrics();
    ...
    private void writeEncoded(CharBuffer charBuffer) throws IOException {
        if (charBuffer.hasRemaining()) {
            ...
            if (this.bbuf == null) {
                this.bbuf = ByteBuffer.allocate(Defaults.RESPONSE_BODY_LIMIT);
            }
            this.encoder.reset();
