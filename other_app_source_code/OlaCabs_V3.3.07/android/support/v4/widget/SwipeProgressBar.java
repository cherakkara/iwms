...
final class SwipeProgressBar {
    ...
    void draw(Canvas canvas) {
        ...
        if (this.mRunning || this.mFinishTime > 0) {
            ...
            if (f >= 0.0f && f <= 25.0f) {
                canvas2 = canvas;
                drawCircle(canvas2, (float) i, (float) i2, this.mColor1, ((25.0f + f) * dm.DEFAULT_BACKOFF_MULT) / 100.0f);
            }
            if (f >= 0.0f && f <= 50.0f) {
                canvas2 = canvas;
                drawCircle(canvas2, (float) i, (float) i2, this.mColor2, (dm.DEFAULT_BACKOFF_MULT * f) / 100.0f);
            }
            if (f >= 25.0f && f <= 75.0f) {
                canvas2 = canvas;
                drawCircle(canvas2, (float) i, (float) i2, this.mColor3, ((f - 25.0f) * dm.DEFAULT_BACKOFF_MULT) / 100.0f);
            }
            if (f >= 50.0f && f <= 100.0f) {
                canvas2 = canvas;
                drawCircle(canvas2, (float) i, (float) i2, this.mColor4, ((f - 50.0f) * dm.DEFAULT_BACKOFF_MULT) / 100.0f);
            }
            if (f >= 75.0f && f <= 100.0f) {
                canvas2 = canvas;
                drawCircle(canvas2, (float) i, (float) i2, this.mColor1, ((f - 75.0f) * dm.DEFAULT_BACKOFF_MULT) / 100.0f);
            }
