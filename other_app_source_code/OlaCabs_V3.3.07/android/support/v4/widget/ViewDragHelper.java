...
public class ViewDragHelper {
    ...
    static class C01101 implements Interpolator {
        C01101() {
        }
    ...
    public static ViewDragHelper create(ViewGroup viewGroup, float f, Callback callback) {
        ...
        create.mTouchSlop = (int) (((float) create.mTouchSlop) * (br.DEFAULT_BACKOFF_MULT / f));
        return create;
    }

    private ViewDragHelper(Context context, ViewGroup viewGroup, Callback callback) {
        this.mActivePointerId = INVALID_POINTER;
        this.mSetIdleRunnable = new C01112();
        if (viewGroup == null) {
    ...
    private int computeAxisDuration(int i, int i2, int i3) {
        ...
        float distanceInfluenceForSnapDuration = (distanceInfluenceForSnapDuration(Math.min(br.DEFAULT_BACKOFF_MULT, ((float) Math.abs(i)) / ((float) width))) * ((float) i4)) + ((float) i4);
        i4 = Math.abs(i2);
        if (i4 > 0) {
    ...
    private void releaseViewForPointerUp() {
        this.mVelocityTracker.computeCurrentVelocity(Constants.MILLIS_IN_A_SECOND, this.mMaxVelocity);
