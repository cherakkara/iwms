...
class MaterialProgressDrawable extends Drawable implements Animatable {
    ...
    class C00931 extends Animation {
        ...
        public void applyTransformation(float f, Transformation transformation) {
            ...
            this.val$ring.setEndTrim(((MaterialProgressDrawable.MAX_PROGRESS_ARC - toRadians) * MaterialProgressDrawable.START_CURVE_INTERPOLATOR.getInterpolation(f)) + startingEndTrim);
            this.val$ring.setStartTrim((MaterialProgressDrawable.END_CURVE_INTERPOLATOR.getInterpolation(f) * MaterialProgressDrawable.MAX_PROGRESS_ARC) + startingStartTrim);
            this.val$ring.setRotation((0.25f * f) + startingRotation);
            MaterialProgressDrawable.this.setRotation((144.0f * f) + (720.0f * (MaterialProgressDrawable.this.mRotationCount / MaterialProgressDrawable.NUM_POINTS)));
        }
    }

    /* renamed from: android.support.v4.widget.MaterialProgressDrawable.2 */
    class C00942 implements AnimationListener {
        final /* synthetic */ Ring val$ring;
        ...
        public void onAnimationRepeat(Animation animation) {
            ...
            MaterialProgressDrawable.this.mRotationCount = (MaterialProgressDrawable.this.mRotationCount + br.DEFAULT_BACKOFF_MULT) % MaterialProgressDrawable.NUM_POINTS;
        }
    }

    /* renamed from: android.support.v4.widget.MaterialProgressDrawable.3 */
    class C00953 implements Callback {
        C00953() {
        }
    ...
    private static class EndCurveInterpolator extends AccelerateDecelerateInterpolator {
        ...
        public float getInterpolation(float f) {
            return super.getInterpolation(Math.max(0.0f, (f - 0.5f) * dm.DEFAULT_BACKOFF_MULT));
        }
    }
    ...
    private static class Ring {
        ...
        private void drawTriangle(Canvas canvas, float f, float f2, Rect rect) {
            if (this.mShowArrow) {
                ...
                this.mArrow.lineTo((((float) this.mArrowWidth) * this.mArrowScale) / dm.DEFAULT_BACKOFF_MULT, ((float) this.mArrowHeight) * this.mArrowScale);
        ...
        public void setInsets(int i, int i2) {
            ...
            if (this.mRingCenterRadius <= 0.0d || min < 0.0f) {
                min = (float) Math.ceil((double) (this.mStrokeWidth / dm.DEFAULT_BACKOFF_MULT));
            } else {
                min = (float) (((double) (min / dm.DEFAULT_BACKOFF_MULT)) - this.mRingCenterRadius);
            }
    ...
    private static class StartCurveInterpolator extends AccelerateDecelerateInterpolator {
        ...
        public float getInterpolation(float f) {
            return super.getInterpolation(Math.min(br.DEFAULT_BACKOFF_MULT, dm.DEFAULT_BACKOFF_MULT * f));
        }
    }
    ...
    private void setupAnimators() {
        Ring ring = this.mRing;
        Animation c00931 = new C00931(ring);
        c00931.setRepeatCount(-1);
        c00931.setRepeatMode(DEFAULT);
        c00931.setInterpolator(LINEAR_INTERPOLATOR);
        c00931.setAnimationListener(new C00942(ring));
        this.mAnimation = c00931;
    }
}
