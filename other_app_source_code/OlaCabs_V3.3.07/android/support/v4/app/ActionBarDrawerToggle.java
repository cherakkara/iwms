...
public class ActionBarDrawerToggle implements DrawerListener {
    ...
    private class SlideDrawable extends InsetDrawable implements Callback {
        ...
        public void draw(Canvas canvas) {
            ...
            if (!(i2 == 0 || this.mHasMirroring)) {
                ...
                canvas.scale(-1.0f, br.DEFAULT_BACKOFF_MULT);
            }
            super.draw(canvas);
            canvas.restore();
        }
    ...
    public void syncState() {
        if (this.mDrawerLayout.isDrawerOpen((int) GravityCompat.START)) {
            this.mSlider.setPosition(br.DEFAULT_BACKOFF_MULT);
        } else {
    ...
    public void onDrawerSlide(View view, float f) {
        ...
        if (f > 0.5f) {
            position = Math.max(position, Math.max(0.0f, f - 0.5f) * dm.DEFAULT_BACKOFF_MULT);
        } else {
            position = Math.min(position, f * dm.DEFAULT_BACKOFF_MULT);
        }
