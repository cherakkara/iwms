...
public class ViewPager extends ViewGroup {
    ...
    static class C00711 implements Comparator<ItemInfo> {
        C00711() {
        }
    ...
    static class C00722 implements Interpolator {
        C00722() {
        }
    ...
    class MyAccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        public void onInitializeAccessibilityEvent(View view, AccessibilityEvent accessibilityEvent) {
            ...
            if (accessibilityEvent.getEventType() == AnalyticAttribute.ATTRIBUTE_VALUE_MAX_LENGTH && ViewPager.this.mAdapter != null) {
        ...
        public void onInitializeAccessibilityNodeInfo(View view, AccessibilityNodeInfoCompat accessibilityNodeInfoCompat) {
            ...
            if (ViewPager.this.canScrollHorizontally(ViewPager.SCROLL_STATE_DRAGGING)) {
                accessibilityNodeInfoCompat.addAction((int) AnalyticAttribute.ATTRIBUTE_VALUE_MAX_LENGTH);
            }
    ...
    public static class SavedState extends BaseSavedState {
        ...
        static class C00741 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C00741() {
            }
        ...
        static {
            CREATOR = ParcelableCompat.newCreator(new C00741());
        }
    ...
    static {
        ...
        LAYOUT_ATTRS = iArr;
        COMPARATOR = new C00711();
        sInterpolator = new C00722();
    ...
    public ViewPager(Context context) {
        ...
        this.mEndScrollRunnable = new C00733();
    ...
    void initViewPager() {
        ...
        this.mCloseEnough = (int) (dm.DEFAULT_BACKOFF_MULT * f);
    ...
    void smoothScrollTo(int i, int i2, int i3) {
        ...
        float distanceInfluenceForSnapDuration = (((float) i6) * distanceInfluenceForSnapDuration(Math.min(br.DEFAULT_BACKOFF_MULT, (((float) Math.abs(i4)) * br.DEFAULT_BACKOFF_MULT) / ((float) clientWidth)))) + ((float) i6);
        int abs = Math.abs(i3);
        if (abs > 0) {
            clientWidth = Math.round(1000.0f * Math.abs(distanceInfluenceForSnapDuration / ((float) abs))) * 4;
        } else {
            clientWidth = (int) (((((float) Math.abs(i4)) / ((((float) clientWidth) * this.mAdapter.getPageWidth(this.mCurItem)) + ((float) this.mPageMargin))) + br.DEFAULT_BACKOFF_MULT) * 100.0f);
        }
    ...
    private void calculatePageOffsets(ItemInfo itemInfo, int i, ItemInfo itemInfo2) {
        ...
        this.mLastOffset = itemInfo.position == count + INVALID_POINTER ? (itemInfo.offset + itemInfo.widthFactor) - br.DEFAULT_BACKOFF_MULT : AutoScrollHelper.NO_MAX;
        ...
        for (i3 = i + SCROLL_STATE_DRAGGING; i3 < size; i3 += SCROLL_STATE_DRAGGING) {
            ...
            if (itemInfo3.position == count + INVALID_POINTER) {
                this.mLastOffset = (itemInfo3.widthFactor + f2) - br.DEFAULT_BACKOFF_MULT;
            }
            itemInfo3.offset = f2;
            f3 = f2 + (itemInfo3.widthFactor + f);
            i2 += SCROLL_STATE_DRAGGING;
        ...
        switch (motionEvent.getAction() & MotionEventCompat.ACTION_MASK) {
            ...
            case SCROLL_STATE_DRAGGING /*1*/:
                if (this.mIsBeingDragged) {
                    ...
                    velocityTracker.computeCurrentVelocity(Constants.MILLIS_IN_A_SECOND, (float) this.mMaximumVelocity);
    ...
    public void draw(Canvas canvas) {
        ...
        if (overScrollMode == 0 || (overScrollMode == SCROLL_STATE_DRAGGING && this.mAdapter != null && this.mAdapter.getCount() > SCROLL_STATE_DRAGGING)) {
            ...
            if (!this.mRightEdge.isFinished()) {
                ...
                canvas.rotate(90.0f);
                canvas.translate((float) (-getPaddingTop()), (-(this.mLastOffset + br.DEFAULT_BACKOFF_MULT)) * ((float) height));
    ...
    public boolean dispatchPopulateAccessibilityEvent(AccessibilityEvent accessibilityEvent) {
        if (accessibilityEvent.getEventType() == AnalyticAttribute.ATTRIBUTE_VALUE_MAX_LENGTH) {
